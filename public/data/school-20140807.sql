...
CREATE TABLE `sch_classes` (
  ...
  `teacher_id` int(9) NOT NULL DEFAULT '0' COMMENT '班主任编号（教师编号）',
  `class_number` int(11) unsigned zerofill NOT NULL DEFAULT '00000000000' COMMENT '班级编码（学校+属性+流水号）',
  `property` tinyint(1) NOT NULL DEFAULT '0' COMMENT '属性（0-普通班1-特长班）',
  `class_no` int(2) unsigned zerofill NOT NULL DEFAULT '00' COMMENT '班级号码（流水号）',
  `class_name` varchar(30) NOT NULL DEFAULT '' COMMENT '班级名称',
  `amount` int(4) NOT NULL DEFAULT '0' COMMENT '人数',
  `create_user_id` int(9) NOT NULL DEFAULT '0' COMMENT '创建者（录入人ID）',
  `create_time_int` int(9) NOT NULL DEFAULT '0' COMMENT '创建时间',
  `comments` text COMMENT '描述',
  `status` tinyint(1) NOT NULL DEFAULT '0' COMMENT '状态（0-未激活1-激活2-合并3-撤销4-升级）',
  `is_delete` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否删除（0-否1-是）',
...
insert  into `sch_classes`(`class_id`,`teacher_id`,`class_number`,`property`,`class_no`,`class_name`,`amount`,`create_user_id`,`create_time_int`,`comments`,`status`,`is_delete`) values (1,1,00000000001,0,01,'普通班',50,1,0,NULL,1,0);
...
/*Table structure for table `sch_special_class_student` */

DROP TABLE IF EXISTS `sch_special_class_student`;

CREATE TABLE `sch_special_class_student` (
  `special_class_student_id` int(9) NOT NULL AUTO_INCREMENT COMMENT '特长班与学生映射编号',
  `special_class_id` int(9) DEFAULT NULL COMMENT '特长班编号',
  `student_id` int(9) DEFAULT NULL COMMENT '学生编号',
  PRIMARY KEY (`special_class_student_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='特长班与学生映射表';

/*Data for the table `sch_special_class_student` */

/*Table structure for table `sch_special_classes` */

DROP TABLE IF EXISTS `sch_special_classes`;

CREATE TABLE `sch_special_classes` (
  `special_class_id` int(9) NOT NULL AUTO_INCREMENT COMMENT '特长班编号',
  `name` varchar(50) DEFAULT NULL COMMENT '名称',
  `teacher_id` int(9) DEFAULT NULL COMMENT '授课老师编号',
  `comments` text COMMENT '简介',
  `class_hour` int(2) DEFAULT NULL COMMENT '课时（一节课时间）',
  `attend_class_address` varchar(50) DEFAULT NULL COMMENT '上课地点',
  `attend_class_id` int(9) DEFAULT NULL COMMENT '上课班级',
  `attend_class_time` datetime DEFAULT NULL COMMENT '上课时间',
  `amount` int(4) DEFAULT NULL COMMENT '人数',
  `open_date` date DEFAULT NULL COMMENT '开班日期',
  `tuition` decimal(11,2) DEFAULT NULL COMMENT '学费',
  PRIMARY KEY (`special_class_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='特长班表';

/*Data for the table `sch_special_classes` */
