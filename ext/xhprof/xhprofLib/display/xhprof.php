...
function xhprof_include_js_css( $ui_dir_url_path = null )
{
	...
	echo "<link href='$ui_dir_url_path/jquery/jquery.facebox.css' " . " rel='stylesheet' type='text/css'></link>";
	//图片预览js，上面是css
	echo "<script src='$ui_dir_url_path/jquery/jquery.facebox.js'>" . "</script>";
	echo <<<EXECUTEJS
  <script type="text/javascript">
    jQuery(document).ready(function($) {
      $('a[id="callsGraph"]').facebox({
        loadingImage : '{$ui_dir_url_path}/jquery/indicator.gif',
        closeImage   : '{$ui_dir_url_path}/jquery/closelabel.png'
      })
    })
  </script>
EXECUTEJS;
...
/*
 * Formats call counts for XHProf reports. Description: Call counts in
 * single-run reports are integer values. However, call counts for aggregated
 * reports can be fractional. This function will print integer values without
 * decimal point, but with commas etc. 4000 ==> 4,000 It'll round fractional
 * values to decimal precision of 3 4000.1212 ==> 4,000.121 4000.0001 ==> 4,000
 */
 ...
 * @param $id raw-str       	
 * @param $title raw-str       	
 * @param $target raw-str       	
 * @param $onclick raw-str       	
 * @param $style raw-str       	
 * @param $access raw-str       	
 * @param $onmouseover raw-str       	
 * @param $onmouseout raw-str       	
 * @param $onmousedown raw-str       	
 * @param $dir raw-str       	
 * @param $rel raw-str       	
 */
...
$sortable_columns = array( 
	"fn" => 1 , "ct" => 1 , "wt" => 1 , "excl_wt" => 1 , "ut" => 1 , "excl_ut" => 1 , "st" => 1 , "excl_st" => 1 , "mu" => 1 , "excl_mu" => 1 , "pmu" => 1 , "excl_pmu" => 1 , "cpu" => 1 , "excl_cpu" => 1 , "samples" => 1 , "excl_samples" => 1 
);
...
function profiler_report( $url_params , $rep_symbol , $sort , $run1 , $run1_desc , $run1_data , $run2 = 0 , $run2_desc = "" , $run2_data = array() )
{
	...
	$links = array();
	$links[] = xhprof_render_link( "查看全部{$diff_text}运行报告" , $top_link_query_string );
	if( $diff_mode )
	{
	...
	echo '<dl class=phprof_report_info>' . '  <dt>' .
			 $diff_text .
			 ' Report</dt>' .
			 '  <dd>' .
			 ( $diff_mode ? $run1_txt .
 ...
 *
 * For instance, negative numbers in diff reports comparing two runs (run1 &
 * run2)
 * represent improvement from run1 to run2. We use green to display those
 * deltas,
...
echo "<tr>";
echo "<th style='text-align:right'>总概要</th>";
echo "<th'></th>";
echo "</tr>";
foreach( $metrics as $metric )
{
	echo "<tr>";
	echo "<td style='text-align:right; font-weight:bold'>全部 " . str_replace( "<br>" , " " , stat_description( $metric ) ) . ":</td>";
...
{
	echo "<tr>";
	echo "<td style='text-align:right; font-weight:bold'>函数调用次数:</td>";
...
print ( "<center><br><h3>" . xhprof_render_link( $callgraph_report_title , "$base_path/callgraph.php" . "?" . http_build_query( $url_params ) . '&pic.jpg' , 'fanbox', 'callsGraph' ) . "</h3></center>" ) ;
$flat_data = array();
...
{
	$title = "显示前{$limit}个函数 : 根据{$desc}排序";
}
}
...
print ( "<br><h4><center>" ) ;
print ( "<b>[$rep_symbol]</b>的父/子函数调用{$regr_impr}报告" ) ;
$callgraph_href = "$base_path/callgraph.php?" . http_build_query( xhprof_array_set( $url_params , 'func' , $rep_symbol ) ) . '&graph.jpg';
print ( " <a href='$callgraph_href' id='callsGraph'>[查看调用图 $diff_text]</a><br>" ) ;
print ( "</center></h4><br>" ) ;
print ( '<table border=1 cellpadding=2 cellspacing=1 width="90%" ' . 'rules=rows bordercolor="#bdc7d8" align=center>' . "\n" ) ;
print ( '<tr bgcolor="#bdc7d8" align=right>' ) ;
...
print ( "<td style='text-align:right;color:blue'>" . "当前函数自身的{$diff_text}资源占用</td>" ) ;
if( $display_calls )
{
// Call Count
print ( "<td $vbar></td>" ) ;
