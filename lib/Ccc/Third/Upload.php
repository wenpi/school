 ...
 * 调用：
 * ------------------
 * @tips:enctype="multipart/form-data"
 * ------------------
 * $savePath=保存路径
 * $fileFormat=文件格式
 * $maxSize=文件大小（kb）
 * $overwrite=覆盖模式(0=不覆盖1=覆盖)
 * $fileInput=表单名称
 * $isChangeName=是否改名（0=不改1=改）
 * -----------------
 * @返回：true=OK|false=NO
 * Ccc_Third_Upload::getInstance( $savePath , $fileFormat , $maxSize , $overwrite = 0 )->run( $fileInput , $isChangeName );
 */
class Ccc_Third_Upload {

    var $saveName; // 保存名
    var $savePath; // 保存路径
    var $fileFormat = array('gif', 'jpg', 'doc', 'application/octet-stream'); // 文件格式&MIME限定
    var $overwrite = 0; // 覆盖模式
    var $maxSize = 0; // 文件最大KB
    var $ext; // 文件扩展名
    var $thumb = 1; // 是否生成缩略图
    var $thumbWidth = 130; // 缩略图宽
    var $thumbHeight = 150; // 缩略图高
    ...
    private function __construct($savePath, $fileFormat = '', $maxSize = 0, $overwrite = 0) {
     ...
     * @return Ccc_Third_Upload
     */
    public static function getInstance($savePath, $fileFormat, $maxSize, $overwrite = 0) {
        $className = __CLASS__;
        if (!isset(self::$_singletonObject[$className]) || !self::$_singletonObject[$className]) {
            self::$_singletonObject[$className] = new self($savePath, $fileFormat, $maxSize, $overwrite);
        }
    ...
    function copyfile($fileArray) {
        ...
        if ($this->maxSize != 0) {
            // 比较KB
            if (round(( $fileArray["size"] / 1024), 2) > $this->maxSize) {
                $this->errno = 14;
                return false;
            }
        }
